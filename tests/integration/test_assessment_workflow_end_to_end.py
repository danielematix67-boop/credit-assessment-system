...
def test_end_to_end_workflow_preserves_cross_macro_critical_decision_to_report() -> None:
    ...
    # The legacy financial-only assessment remains NORMAL. The final decision
    # must come from the case-level final assessment and flow unchanged through
    # analysis, reporting, and the UI-facing workflow result.
    assert result.assessment.status is AssessmentStatus.NORMAL
