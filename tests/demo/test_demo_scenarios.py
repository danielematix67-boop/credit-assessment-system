...
EXPECTED_FINAL_STATUS = {
    ...
    "Profitability Stress": "ATTENTION",
...
def test_demo_scenarios_have_expected_final_statuses() -> None:
...
def test_demo_case_analysis_agent_preserves_final_assessment_status() -> None:
    case_service = CreditAssessmentCaseService(
        assessment_service=AssessmentService(),
        final_assessment_service=FinalAssessmentService(),
    )
...
def test_missing_information_preserves_explicit_limitations() -> None:
    position = build_demo_position("Missing Information")
    profile, behavioural, debt = build_demo_case_data("Missing Information")

    case = CreditAssessmentCaseService(assessment_service=AssessmentService()).assess(
        position,
        customer_profile_data=profile,
        behavioural_data=behavioural,
        debt_sustainability_data=debt,
