...
def make_finding(text: str = "Test finding.", *, rule_id: str = "TEST_RULE", category: str = "Test Category", severity: RuleSeverity = RuleSeverity.MEDIUM) -> AnalysisFinding:
...
def test_local_llm_falls_back_when_indicator_values_are_missing() -> None:
    ...
    generator, _ = make_generator(response="EBITDA is negative at €-120,000 and leverage stands at 7.0x.", require_indicator_values=True)
    report = generator.generate(analysis)
...
def test_discursive_narrative_preserves_all_findings_and_indicators() -> None:
    findings = [
        make_finding("Revenue growth declined to -20.0%, indicating a deterioration in the company's top-line performance.", category="Revenue"),
        make_finding("EBITDA is negative, with a reported value of €-120,000, indicating negative operating profitability.", category="Profitability"),
        make_finding("The interest coverage ratio is -0.2x, indicating that EBITDA provides insufficient coverage of interest expense.", category="Profitability"),
        make_finding("EBITDA margin is -8.0%.", category="Profitability"),
        make_finding("NFP to EBITDA stands at 7.0x, above the acceptable level, indicating elevated leverage relative to operating earnings.", category="Leverage"),
    ]
    analysis = make_analysis(status=AssessmentStatus.CRITICAL, key_findings=findings)
    response = (
