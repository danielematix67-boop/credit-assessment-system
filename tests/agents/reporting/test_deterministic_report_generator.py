...
def findings_by_category_from_analysis(analysis: AssessmentAnalysis) -> list[ReportFindingGroup]:
...
def test_deterministic_report_generator_preserves_analysis_data(generator):
    findings = [
        make_analysis_finding(
            rule_id=f"RULE_{index}", category=f"category_{index}", severity=severity, text=f"Finding {index}."
        )
        for index, severity in enumerate(RuleSeverity, start=1)
    ]
    limitations = [
        make_analysis_finding(
            rule_id=f"LIMITATION_{index}", category=f"category_{index}", severity=severity, text=f"Limitation {index}."
        )
        for index, severity in enumerate(RuleSeverity, start=1)
    ]
    analysis = make_analysis(
        position_id="CUSTOM_POSITION", status=AssessmentStatus.ATTENTION,
        key_findings=findings, limitations=limitations,
    )
...
def test_deterministic_report_generator_generates_summary_for_each_status(generator, status):
...
def test_deterministic_report_generator_groups_findings_by_category(generator):
    findings = [
        make_analysis_finding(rule_id=f"RULE_{index}", category=category, severity=RuleSeverity.MEDIUM, text=f"Finding {index}.")
        for index, category in enumerate(("category_a", "category_b", "category_b", "category_c"), start=1)
    ]
...
def test_deterministic_report_generator_preserves_arbitrary_content(generator):
    key_findings = [
        make_analysis_finding(rule_id="ARBITRARY_RULE_A", category="arbitrary_category_a", severity=RuleSeverity.LOW, text="Arbitrary finding A."),
        make_analysis_finding(rule_id="ARBITRARY_RULE_B", category="arbitrary_category_b", severity=RuleSeverity.HIGH, text="Arbitrary finding B."),
    ]
    risk_factors = [make_analysis_finding(rule_id="ARBITRARY_RISK", category="arbitrary_risk_category", severity=RuleSeverity.HIGH, text="Arbitrary risk.")]
    limitations = [make_analysis_finding(rule_id="ARBITRARY_LIMITATION", category="arbitrary_limitation_category", severity=RuleSeverity.MEDIUM, text="Arbitrary limitation.")]
    analysis = make_analysis(position_id="ARBITRARY_POSITION", status=AssessmentStatus.NORMAL, key_findings=key_findings, risk_factors=risk_factors, limitations=limitations)
    report = generator.generate(analysis)
    assert report.position_id == analysis.position_id
    assert report.assessment_status == analysis.assessment_status
    assert report.findings_by_category == findings_by_category_from_analysis(analysis)
