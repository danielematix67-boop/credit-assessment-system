...
def test_r002_negative_ebitda_triggers():
    result = NegativeEbitdaRule(CONFIG).evaluate(
        CreditPosition(position_id="P", ebitda=-50_000)
    )
...
def test_r002_positive_ebitda_does_not_trigger():
    result = NegativeEbitdaRule(CONFIG).evaluate(
        CreditPosition(position_id="P", ebitda=250_000)
    )
...
def test_r002_none_is_not_evaluable():
    result = NegativeEbitdaRule(CONFIG).evaluate(
        CreditPosition(position_id="P", ebitda=None)
    )
