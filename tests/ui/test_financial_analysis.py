...
def test_indicator_analysis_frame_exposes_deterministic_evidence() -> None:
    ...
    assert frame.loc[2, "Value"] == 3.00
    assert frame.loc[2, "Threshold"] == 5.00


def test_indicator_analysis_frame_excludes_non_evaluable_values() -> None:
    section = SimpleNamespace(
        evidence=[_rule("R001", None, 0.00, "NOT_EVALUABLE", "EBITDA Margin")]
    )
