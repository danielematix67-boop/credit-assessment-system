...
class DebtSustainabilityAssessmentService:
    ...
    def assess(self, data: DebtSustainabilityData) -> AssessmentSection:
        ...
        if all(result.status == RuleStatus.NOT_EVALUABLE for result in results):
            limitations.append("Debt-service and cash-flow data are not available.")
        else:
            limitations.append(
    ...
    def _dscr_result(cls, data: DebtSustainabilityData) -> RuleResult:
        ...
        if data.debt_service <= 0:
            return cls._not_evaluable(
                "DS001",
                "Debt Service Coverage Ratio",
                "Debt service must be positive to calculate DSCR.",
                cls._DSCR_THRESHOLD,
                SeverityDirection.LOWER_IS_WORSE,
            )

        value = data.cash_flow_available_for_debt_service / data.debt_service
        return cls._lower_is_worse_ratio(
            "DS001",
            "Debt Service Coverage Ratio",
            "DSCR",
            value,
            cls._DSCR_THRESHOLD,
            cls._DSCR_HIGH_THRESHOLD,
        )
    ...
    def _debt_service_to_ebitda_result(cls, data: DebtSustainabilityData) -> RuleResult:
        if data.debt_service is None or data.ebitda is None:
            return cls._not_evaluable(
                ...
                "Debt Service / EBITDA",
                "Debt Service / EBITDA",
                cls._DEBT_SERVICE_TO_EBITDA_THRESHOLD,
        ...
        return cls._higher_is_worse_ratio(
            "DS002",
            "Debt Service / EBITDA",
            "Debt Service / EBITDA",
            value,
            cls._DEBT_SERVICE_TO_EBITDA_THRESHOLD,
            cls._DEBT_SERVICE_TO_EBITDA_HIGH_THRESHOLD,
        )
    ...
    def _cash_flow_buffer_result(cls, data: DebtSustainabilityData) -> RuleResult:
        if data.cash_flow_available_for_debt_service is None or data.debt_service is None:
            return cls._not_evaluable(
                "DS003",
                "Cash Flow Debt-Service Buffer",
                "CFADS - debt service",
                cls._CASH_FLOW_BUFFER_THRESHOLD,
                SeverityDirection.LOWER_IS_WORSE,
            )
        ...
        return cls._lower_is_worse_ratio(
            ...
            value,
            cls._CASH_FLOW_BUFFER_THRESHOLD,
            cls._CASH_FLOW_BUFFER_HIGH_THRESHOLD,
        )

    @staticmethod
    def _lower_is_worse_ratio(
        rule_id: str,
        rule_name: str,
        indicator: str,
        value: float,
