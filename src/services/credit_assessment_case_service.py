...
class CreditAssessmentCaseService:
    ...
    _FINANCIAL_DIMENSIONS = {
        "R001": "Revenue & Growth", "R002": "Profitability", "R003": "Profitability",
        "R004": "Financial Structure", "R005": "Debt Service Burden",
        "R006": "Profitability Quality", "R007": "Debt Service Burden",
    }
    ...
    def assess(
        ...
        behavioural_section = (
            ...
            if behavioural_data is not None else self._not_evaluable_section(
                "Behavioural Analysis", "Behavioural banking data are not available for this case."
            )
        )
        debt_sustainability_section = (
            self.debt_sustainability_assessment_service.assess(debt_sustainability_data)
            if debt_sustainability_data is not None else self._not_evaluable_section(
                "Debt Sustainability", "Debt-service and cash-flow data are not available for this case."
            )
        )
        customer_profile_section = (
            self.customer_profile_assessment_service.assess(customer_profile_data)
            if customer_profile_data is not None else self._not_evaluable_section(
                "Customer Profile", "Customer profile data are not available for this case."
            )
        )
    ...
    def _financial_section(cls, assessment: Assessment) -> AssessmentSection:
        ...
        return AssessmentSection(
            name="Financial Analysis", status=SectionStatus(assessment.status.value),
            findings=assessment.findings, evidence=assessment.rule_results,
            limitations=[], dimensions=dimensions,
        )

    @staticmethod
    def _not_evaluable_section(name: str, limitation: str) -> AssessmentSection:
        return AssessmentSection(
            name=name, status=SectionStatus.NOT_EVALUABLE, findings=[], evidence=[], limitations=[limitation]
        )
