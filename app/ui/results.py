...
def render_result_hero(result: Any) -> None:
    ...
    st.caption(
        ...
        "The Executive Report is the final user-facing output and can be generated "
        "deterministically, with Ollama, or with Gemini."
    )
    ...
    with st.container(border=True):
        ...
        with result_col:
            ...
            st.caption(
                "Read the Executive Report first. The sections below provide the "
                "evidence and audit trail supporting it."
            )
...
def render_results(
    ...
    # ========================================================
    # LEVEL 1 — EXECUTIVE SUMMARY + FINAL OUTPUT
    # ========================================================
    render_result_hero(result)

    # The Executive Report is the primary business output of the application.
    # It is deliberately displayed before the supporting risk analysis.
    with st.container(border=True):
        st.markdown("## Executive Report — Final Output")
        st.caption(
            "This is the final report produced from the structured assessment. "
            "The badge below identifies whether the report was generated by the "
            "deterministic engine, Ollama, or Gemini."
        )
        render_report_tab(
            result=result,
            selected_reporting_mode=selected_reporting_mode,
    ...
    st.caption(
        ...
        "execution path behind the Executive Report."
    )
    ...
    with st.expander("Additional Views", expanded=False):
        st.caption(
            ...
            "inspection. The Executive Report above remains the primary output."
        )

        overview_tab, findings_tab, analysis_tab = st.tabs(
            [
                "Overview",
                "🔒 Rule Findings",
                "🔒 Analysis",
