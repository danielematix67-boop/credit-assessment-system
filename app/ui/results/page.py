...
def _apply_results_styles() -> None:
    st.markdown(
        ...
        .results-kpi-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: .65rem; margin: .65rem 0 1rem 0; }
        ...
        @media (max-width: 760px) { .assessment-hero-top { flex-direction: column; } .results-kpi-grid { grid-template-columns: 1fr; } }
        </style>
        """,
...
def render_results_header(result: Any) -> None:
    ...
    st.markdown(
        ...
        <div class="assessment-hero">
            ...
            <div class="assessment-subtitle">Deterministic assessment with reporting used only for executive narrative.</div>
        </div>
...
def render_reporting_provenance(result: Any) -> None:
    ...
    st.markdown(
        f'<div class="reporting-provenance">Narrative reporting: {mode or "Unknown"} · {generator_label}.</div>',
        unsafe_allow_html=True,
    )
