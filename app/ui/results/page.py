...
def _apply_results_styles() -> None:
    st.markdown(
        ...
        .reporting-status { margin: .2rem 0 1rem 0; padding: .9rem 1rem; border: 1px solid rgba(128,128,128,.18); border-radius: .8rem; background: rgba(128,128,128,.025); }
        .reporting-status-title { font-size: .82rem; font-weight: 750; letter-spacing: .04em; text-transform: uppercase; }
        .reporting-status-note { margin-top: .25rem; color: rgba(128,128,128,.92); font-size: .78rem; line-height: 1.4; }
        .reporting-status-used { font-weight: 750; }
...
def render_reporting_status(result: Any) -> None:
    """Show which reporting generator actually produced the narrative."""
    metadata = getattr(result, "execution_metadata", None)
    mode = getattr(metadata, "reporting_mode", None) or getattr(result, "reporting_mode", None) or "Unknown"
    generator = getattr(metadata, "generator_used", None) or getattr(result, "report_generator_used", None)
    fallback_used = bool(
        getattr(metadata, "fallback_used", False)
    ...
    if generator == "FALLBACK" or fallback_used:
        title = "Fallback used"
        used_label = "Deterministic fallback"
        note = "The primary reporting generator failed; the report was generated by the deterministic fallback."
    elif generator == "PRIMARY":
        title = "Primary reporting"
        used_label = "Primary generator"
        note = "The selected reporting generator completed successfully; no fallback was required."
    elif mode == "Deterministic":
        title = "Deterministic reporting"
        used_label = "Deterministic generator"
        note = "The report was generated deterministically; no LLM reporting path was requested."
    else:
        title = "Reporting status"
        used_label = generator or "Not available"
        note = "Reporting provenance is available from the workflow execution metadata."

    st.markdown(
        f"""
        <div class="reporting-status">
            <div class="reporting-status-title">{title}</div>
            <div class="reporting-status-note">Mode: <span class="reporting-status-used">{mode}</span> · Generator: <span class="reporting-status-used">{used_label}</span></div>
            <div class="reporting-status-note">{note}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    if fallback_used:
        details = ""
        if error_category:
            details += f"Error category: {error_category}. "
        if error_message:
            details += f"Primary generation error: {error_message}"
        if details:
            st.caption(details)
...
def render_results(result: Any, assessment_position: Any, selected_reporting_mode: str) -> None:
    ...
    render_reporting_status(result)
